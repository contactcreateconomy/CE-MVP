...
const rankColors = {
  1: { ring: "var(--rank-gold)", icon: "var(--rank-gold)" },
  2: { ring: "var(--rank-silver)", icon: "var(--rank-silver)" },
  3: { ring: "var(--rank-bronze)", icon: "var(--rank-bronze)" },
  4: { ring: "var(--rank-four)", icon: "var(--rank-four)" },
  5: { ring: "var(--rank-five)", icon: "var(--rank-five)" },
} as const;
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="relative w-[160px] rounded-full border border-(--border-default) bg-(--bg-overlay)/50 p-1">
          <div
            className="pointer-events-none absolute bottom-1 left-1 top-1 w-[calc((100%-0.5rem)/3)] rounded-full bg-(--brand-primary) transition-transform duration-slow ease-out shadow-glow-primary-pill"
            ...
            {filters.map((filter) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "h-6 rounded-full px-2 text-micro font-semibold transition-colors duration-normal",
      ...
      <div className="space-y-2">
        {topRows.map((row) => {
          ...
          return (
            <div
              key={row.userId}
              className={`group relative flex items-center justify-between overflow-hidden rounded-lg border border-(--border-subtle) bg-(--bg-overlay)/45 px-3 py-2.5 ${
      ...
      <div className="flex w-full justify-center">
        <Link
          ...
          className="inline-flex h-8 w-8 items-center justify-center rounded-full text-(--brand-primary) transition-all duration-normal hover:bg-(--bg-overlay)/55 hover:text-(--brand-primary-hover) hover:shadow-glow-primary-sm"
        >
