...
export function TopPostHeroCarousel({ slides, className }: TopPostHeroCarouselProps) {
  ...
  return (
    <motion.div
      ref={heroRef}
      className={cn(
        "group/hero relative overflow-hidden rounded-hero bg-(--bg-surface) select-none",
        className,
      )}
                    ...
                    >
                      ...
                      {isCenter && (
                        <AnimatePresence mode="wait">
                          ...
                          >
                            <p className="text-micro font-bold uppercase tracking-[0.18em] text-(--brand-primary) mb-1">
                              {active.eyebrow}
                            </p>
            ...
            >
              ...
              <Link
                ...
                className={cn(
                  ...
                  "border border-(--border-default) transition-all duration-slow",
                  "text-(--text-primary)",
...
export function TopPostHeroCarouselSkeleton({ className }: { className?: string }) {
  return (
    <div
      className={cn(
        "relative overflow-hidden rounded-hero border border-(--border-subtle) bg-(--bg-surface)",
        className,
      )}
    >
      <div className="absolute inset-0 z-0 bg-(--bg-overlay)/10 animate-pulse" />
      <div className="absolute inset-0 z-10 flex items-center justify-center">
        <div className="relative h-[275px] w-full max-w-[549px] overflow-hidden rounded-modal-auth bg-(--bg-overlay)/40 animate-pulse shadow-sm">
...
export function TopPostHeroCarouselEmpty({ className }: { className?: string }) {
  return (
    ...
    >
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,var(--glow-primary-halo),transparent_52%)]" />
