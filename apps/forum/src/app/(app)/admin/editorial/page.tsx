 ...
 * A10 v1 per DECISIONS-LOCKED #10: two-pane layout — draft LEFT, source
 * evidence RIGHT, click-a-claim syncs highlighting; Approve disabled until
 * every claim has been checked at least once; mobile = Draft/Evidence tabs
 * instead of side-by-side. Approve's mutation is P4-10 — the affordance
 * renders disabled with its gate reason, server enforcement lands there
 * (canApprove is the shared invariant).
 */
...
export default function AdminEditorialPage() {
  ...
  const review = useQuery(
    ...
    selectedId ? { candidateId: selectedId } : "skip",
  );
      ...
      {review ? (
        <Card>
          <CardHeader className="flex flex-row items-center justify-between p-4 pb-2">
            ...
            {/* CAP-043 affordance: disabled until every claim confirmed; the
                server gate itself is P4-10's approve consuming canApprove. */}
            <Button
              variant="primary"
              size="sm"
              disabled={!review.approveGate}
              title={review.approveGate ? "Approve (lands with P4-10)" : "Approve is fail-closed until every claim is confirmed (CAP-043)"}
            >
              Approve
            </Button>
          </CardHeader>
